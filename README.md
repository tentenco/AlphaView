...
- **Compare research candidates.** Select 2–5 known stocks over 60, 120, or 252 trading-day intervals, with a shared starting date, adjusted-price percentage curves, and explicit coverage. Incomplete series remain listed with reasons and are not plotted. Export the submitted summary and daily percentage series with source, version and stale-context flags.
...
| Relative strength | Average percentile rank of 120-session returns ≥ 80 within the eligible same-date pool, and close ≥ 90% of its 120-session high (including today) |

Relative volume divides current volume by the preceding 20-session simple average, excluding today. Moving averages include today and require their full 20/50/200 observations. Wilder RSI(14) seeds average gains/losses from 14 changes (15 closes), then uses `(13 × previous average + current change) / 14`; flat history is 50, gains without losses are 100. RPS uses ascending average ranks for ties divided by eligible peer count × 100; peers require 121 prices, a current same-date close, valid history and finite required metrics, with at least three peers. A score ≥ 80 does not necessarily select exactly 20% of a small or tied pool.
...
Each new scan stores the scanner engine version and input revision captured with its price histories and membership. The current scanner is `alphaview-scan-v1`; changes to signal semantics require a new version under the [scanner version policy](docs/scan-provenance.md). Publishing a scan does not change that revision; an input change during calculation prevents publication. Legacy snapshots without complete version provenance remain available for review but require recalculation before their signals can be paired with current charts. A workspace-wide input counter is deliberately conservative: a change outside the selected scope can also require a rescan.
