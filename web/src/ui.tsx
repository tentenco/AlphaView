...
export function Delta({
  ...
  return (
    <span className={value == null || value === 0 ? 'muted' : value > 0 ? 'positive' : 'negative'}>
      {value != null &&
        value !== 0 &&
        (value > 0 ? <ArrowUpRight size={16} /> : <ArrowDownRight size={16} />)}
      {percent ? pct(value) : `${value != null && value > 0 ? '+' : ''}${num(value)}`}
    </span>
  )
}
export function Badge({ signal }: { signal: Signal }) {
  ...
  return (
    <details className="signal-badge-details">
      <summary
        className={`badge ${signal.status}`}
        aria-label={`${labels[signal.status]}，查看策略原因`}
      >
        {signal.matched ? <Checkmark size={12} /> : <i />}
        {labels[signal.status]}
      </summary>
      <p className="signal-badge-reason">{signal.reason || '尚無策略原因說明。'}</p>
    </details>
  )
}
    ...
    >
      <div className="dialog-content">
        <header>
          ...
          <button type="button" className="icon-button" aria-label="關閉視窗" onClick={onClose}>
            <Close size={20} />
          </button>
        </header>
        {children}
