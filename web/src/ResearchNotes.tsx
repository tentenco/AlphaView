...
export function ResearchNotes({
  ...
  const dirty =
    currentSaved !== null &&
    (note !== currentSaved.note || JSON.stringify(tagList) !== JSON.stringify(currentSaved.tags))
  const validation =
    note.length > 6000
      ? '筆記最多 6,000 個字元。'
      : tagList.length > 5
        ? '最多可設定 5 個標籤。'
        : tagList.some((tag) => tag.length > 24)
          ? '每個標籤最多 24 個字元。'
          : ''
  useEffect(() => {
  ...
  async function save() {
    ...
    try {
      ...
      if (response.status === 409) {
        setConflict(true)
        setError(
          '這份筆記已在其他視窗更新。你的草稿仍保留，請複製需要的文字，再決定是否載入最新版本。',
        )
        return
      }
  ...
  return (
    <section className="research-note" aria-labelledby={`${id}-title`}>
      <div className="section-heading">
        <div>
          <h3 id={`${id}-title`}>研究筆記（本機儲存）</h3>
          <p>{symbol} · 記錄自己的研究依據與待確認事項。</p>
        </div>
        <span className="quiet-label">
          {dirty
            ? '有尚未儲存的變更'
            : currentSaved?.updated_at
              ? `${dateTime(currentSaved.updated_at)} 儲存`
              : '尚未儲存筆記'}
        </span>
      </div>
      ...
      {currentSaved && (
        ...
        >
          <label htmlFor={`${id}-note`}>研究內容</label>
          <textarea
            id={`${id}-note`}
            name="research-note"
            rows={6}
          ...
          />
          <p id={`${id}-count`} className="footnote">
            {note.length.toLocaleString('en-US')} / 6,000 字元 ·
            筆記由你輸入，不會自動產生投資結論。
          </p>
          <label htmlFor={`${id}-tags`}>研究標籤</label>
          <input
            id={`${id}-tags`}
            name="research-tags"
            value={tags}
            ...
            placeholder="例如：財報追蹤, 等待突破"
            style={{ display: 'block', width: '100%', marginTop: 8 }}
          />
          <p id={`${id}-tag-help`} className="footnote">
            以逗號分隔，最多 5 個標籤，每個最多 24 個字元。
          </p>
          {validation && (
          ...
          <div className="dialog-actions">
            <button
              ...
              onClick={() => {
                setNote(currentSaved.note)
                setTags(currentSaved.tags.join(', '))
                setMessage('已取消本次變更。')
                setError('')
                setConflict(false)
              }}
            ...
            <button
              ...
              disabled={!dirty || loading || saving || !!validation || conflict}
            >
              {saving ? '儲存筆記中…' : '儲存研究筆記'}
            </button>
          </div>
