...
const item = (symbol: string, status: QualityStatus = 'stale'): QualityItem => ({
  ...
  bars: 249,
  reason: `${symbol} diagnostic`,
  gap_dates: ['2026-08-31'],
  invalid_dates: [],
...
describe('data quality coverage', () => {
  it('shows expected session, filters issues, exposes gap dates and opens the selected symbol', async () => {
    vi.stubGlobal(
      'fetch',
      vi.fn().mockImplementation(() => Promise.resolve(response(quality()))),
    )
    ...
    expect(
      (screen.getByRole('checkbox', { name: '選取 NVDA 重試' }) as HTMLInputElement).disabled,
    ).toBe(true)
  ...
  it('retries selected issues and refreshes coverage after the callback completes', async () => {
    ...
    expect(
      (screen.getByRole('checkbox', { name: '選取 DELL 重試' }) as HTMLInputElement).checked,
    ).toBe(false)
  })
  ...
  it('limits cross-page batch selections to 100 symbols', async () => {
    ...
    vi.stubGlobal(
      'fetch',
      vi.fn().mockImplementation(() => Promise.resolve(response(quality(items)))),
    )
    const callbacks = props()
    render(<DataQuality {...callbacks} />)
    ...
    expect(
      (screen.getByRole('checkbox', { name: '選取 STOCK100 重試' }) as HTMLInputElement).disabled,
    ).toBe(true)
    expect(
      (screen.getByRole('checkbox', { name: '選取本頁問題標的' }) as HTMLInputElement).disabled,
    ).toBe(true)
    await userEvent.click(screen.getByRole('checkbox', { name: '選取本頁問題標的' }))
  ...
  it('keeps selections after a failed retry and blocks retries during a global job', async () => {
    ...
    const callbacks = props()
    callbacks.onRetry.mockRejectedValue(new Error('Service unavailable'))
    ...
    expect(
      (screen.getByRole('button', { name: '重試選取資料（1）' }) as HTMLButtonElement).disabled,
    ).toBe(true)
    await act(async () => {})
  })
  ...
  it('recovers from a load failure and ignores superseded responses', async () => {
    ...
    const fetcher = vi
      ...
      .mockResolvedValueOnce(response(quality([item('FRESH', 'error')])))
    vi.stubGlobal('fetch', fetcher)
    const callbacks = props()
    const view = render(<DataQuality {...callbacks} />)
    view.rerender(<DataQuality {...callbacks} busy />)
    ...
    await act(async () => {
      old.resolve(response(quality([item('OLD')])))
    })
  ...
  it('offers reload after an endpoint failure without pretending the data is healthy', async () => {
    vi.stubGlobal(
      ...
      vi
        ...
        .mockResolvedValueOnce(response(quality([]))),
    )
