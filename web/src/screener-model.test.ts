...
const row = (symbol: string, rsi: number | null, extra = {}): Research => ({
  symbol,
  date: '2026-09-04',
  bars: 250,
  indicators: { close: 100, rsi, ...extra },
  signals: [],
})
describe('screener numeric filters and sorting', () => {
  it('keeps missing unrelated metrics but excludes missing or non-finite filtered metrics', () => {
    ...
    expect(
      filterRows(rows, settings({ numeric: { ...EMPTY_NUMERIC, priceMin: '20' } }), new Set()),
    ).toHaveLength(4)
    expect(
      filterRows(rows, settings({ numeric: { ...EMPTY_NUMERIC, rsiMin: '0' } }), new Set()).map(
        (r) => r.symbol,
      ),
    ).toEqual(['GOOD'])
  })
  it.each(['asc', 'desc'] as const)(
    'sorts missing values last in %s order with stable symbol ties',
    (direction) => {
      const sorted = filterRows(
        [row('NULL', null), row('B', 50), row('A', 50), row('LOW', 10)],
        settings({ direction }),
        new Set(),
      )
      expect(sorted.map((r) => r.symbol)).toEqual(
        direction === 'asc' ? ['LOW', 'A', 'B', 'NULL'] : ['A', 'B', 'LOW', 'NULL'],
      )
    },
  )
  it('validates bounds, finite values and integral match count', () => {
    for (const extra of [
      ...
      expect(validateNumeric({ ...EMPTY_NUMERIC, ...extra })).toBeTruthy()
    expect(
      validateNumeric({ ...EMPTY_NUMERIC, rsiMin: '0', rsiMax: '100', matchesMin: '4' }),
    ).toBeNull()
    expect(
      filterRows(
        [row('A', 50)],
        settings({ numeric: { ...EMPTY_NUMERIC, rpsMin: 'NaN' } }),
        new Set(),
      ),
    ).toEqual([])
  })
  it('applies new-only only in market scope and selected matching strategy', () => {
    const hit = {
      ...row('A', 50),
      signals: [{ strategy: 'trend', matched: true, status: 'match' as const, reason: 'match' }],
    }
...
describe('saved presets', () => {
  ...
  it('accepts validated version-one records and rejects corrupt, unknown or duplicated records', () => {
    ...
    const bad = [
      { ...good, version: 2 },
      { ...good, name: '' },
      { ...good, settings: { ...settings(), scope: 'all' } },
      { ...good, settings: { ...settings(), numeric: { ...EMPTY_NUMERIC, rsiMin: '200' } } },
      { ...good, settings: { ...settings(), only: 'true' } },
      { ...good, settings: { ...settings(), sort: '__proto__' } },
    ]
    expect(decodePresets(JSON.stringify([...bad, good, good]))).toEqual([good])
    for (const raw of [null, '{broken', '{}', 'null', '[null, 1, "x"]'])
      expect(decodePresets(raw)).toEqual([])
  })
})
describe('CSV export', () => {
  ...
  it('exports all filtered rows with provenance rather than a 25-row page', () => {
    const rows = Array.from({ length: 32 }, (_, i) => ({
      ...row(`S${i}`, 50),
      name: i === 0 ? '=evil()' : 'Company',
    }))
    const csv = screenerCsv(filterRows(rows, settings(), new Set()), {
      scope: 'market',
      asOf: '2026-09-04',
