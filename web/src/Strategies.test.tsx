...
describe('strategy research request lifecycle', () => {
  it('keeps a fresh POST result when an older cached GET arrives afterwards', async () => {
    ...
    await act(async () => {
      posted.resolve(response(backtest('NVDA', 'turtle', 'Fresh POST result')))
    })
    expect(await screen.findByText(/Fresh POST result/)).toBeTruthy()
    await act(async () => {
      cached.resolve(response(backtest('NVDA', 'turtle', 'Old cached result')))
    })
  ...
  it('ignores out-of-order symbol and strategy responses and includes market candidates only once', async () => {
    ...
    await act(async () => {
      third.resolve(response(backtest('DELL', 'trend', 'Current DELL trend')))
    })
    expect(await screen.findByText(/Current DELL trend/)).toBeTruthy()
    await act(async () => {
      first.resolve(response(backtest('NVDA', 'turtle', 'Old NVDA')))
      second.resolve(response(backtest('DELL', 'turtle', 'Old DELL')))
    })
  ...
  it('locks strategy navigation while a run is pending and recovers after an API failure', async () => {
    ...
    vi.stubGlobal(
      'fetch',
      vi.fn().mockResolvedValueOnce(response(null)).mockReturnValueOnce(posted.promise),
    )
    render(<Strategies data={overview()} />)
    await userEvent.click(screen.getByRole('button', { name: '執行回測' }))
    expect(
      (screen.getByRole('button', { name: /均線趨勢.*trend/ }) as HTMLButtonElement).disabled,
    ).toBe(true)
    expect((screen.getByRole('combobox', { name: '研究標的' }) as HTMLSelectElement).disabled).toBe(
      true,
    )
    await act(async () => {
      posted.resolve(new Response(JSON.stringify({ detail: '日線資料不足' }), { status: 400 }))
    })
    expect(await screen.findByRole('alert')).toHaveProperty('textContent', '日線資料不足')
    expect((screen.getByRole('button', { name: '執行回測' }) as HTMLButtonElement).disabled).toBe(
      false,
    )
    expect(
      (screen.getByRole('button', { name: /均線趨勢.*trend/ }) as HTMLButtonElement).disabled,
  ...
  it('aborts an active POST on unmount and accepts a late response without a state update', async () => {
    ...
    await act(async () => {
      posted.resolve(response(backtest()))
    })
  ...
  it('falls back to available market members when NVDA is absent and handles an empty universe', async () => {
    ...
    const data = overview()
    data.positions = []
    data.market_universe = data.market_universe.filter((m) => m.symbol === 'DELL')
    const view = render(<Strategies data={data} />)
    expect((screen.getByRole('combobox', { name: '研究標的' }) as HTMLSelectElement).value).toBe(
      'DELL',
    )
...
describe('backtest parameters and diagnostics', () => {
  it('sends explicit parameters and uses them in saved-result lookup', async () => {
    const fetcher = vi
      ...
      .mockImplementation((_url: string, init?: RequestInit) =>
        Promise.resolve(response(init?.method === 'POST' ? backtest() : null)),
      )
    vi.stubGlobal('fetch', fetcher)
    render(<Strategies data={overview()} />)
    fireEvent.change(screen.getByRole('spinbutton', { name: '起始資金（USD）' }), {
      target: { value: '25000' },
    })
    fireEvent.change(screen.getByRole('spinbutton', { name: '單邊交易成本（bps）' }), {
      target: { value: '25' },
    })
    ...
    expect(fetcher.mock.calls.at(-1)?.[0]).toContain(
      'initial=25000&fee_bps=25&start_date=2025-01-01&end_date=2026-01-01',
    )
    await userEvent.click(screen.getByRole('button', { name: '執行回測' }))
  ...
  it('blocks invalid capital, costs and reversed dates', async () => {
    vi.stubGlobal(
      'fetch',
      vi.fn().mockImplementation(() => Promise.resolve(response(null))),
    )
    render(<Strategies data={overview()} />)
    fireEvent.change(screen.getByRole('spinbutton', { name: '起始資金（USD）' }), {
      target: { value: '0' },
