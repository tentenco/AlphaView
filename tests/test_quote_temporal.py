...
def test_weekend_stock_uses_matching_prior_session_scan(client):
    ...
    with store.connect() as db:
        db.execute("INSERT INTO scans(created_at,as_of,universe,result,scope,input_revision) VALUES ('test','2024-01-05','[\"A\"]',?,'portfolio',?)",(json.dumps([row]),scan_provenance.current_token(db)))
