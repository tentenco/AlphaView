...
STRATEGIES = [
    {"id": "turtle", "name": "海龜突破", "english": "Turtle breakout", "period": 21,
     ...
     "rules": ["收盤 > 前 20 日最高價（不含當日）", "收盤 > 開盤且高於前日收盤", "成交量 ≥ 前 20 日均量（不含當日）"],
     "origin": "以價格突破、陽線與相對成交量確認訊號。"},
    {"id": "trend", "name": "均線趨勢", "english": "Trend following", "period": 200,
     "description": "辨識長期上升趨勢中，價格與成交量同步轉強的標的。",
     "rules": ["收盤 > MA50 > MA200", "成交量 ≥ 前 20 日均量的 1.2 倍（不含當日）"],
     "origin": "MA 採含當日的完整簡單平均，分別需 50／200 筆；另要求成交量確認。"},
    {"id": "pullback", "name": "回檔觀察", "english": "RSI pullback", "period": 200,
     ...
     "origin": "RSI 以首 14 次漲跌的平均值初始化，需 15 筆收盤；其後採 Wilder 平滑。全平盤為 50，只有上漲為 100；訊號另需 MA200 與轉強條件。"},
    {"id": "rps", "name": "相對強勢", "english": "Relative strength", "period": 121,
     "description": "比較所選股票池內 120 日報酬，找出接近區間高點的領先標的。",
     "rules": ["120 日報酬的 RPS ≥ 80（平均名次／有效檔數 × 100）", "收盤 ≥ 120 日最高價的 90%（含當日）", "至少 3 檔具足夠歷史資料"],
     "origin": "排名僅限所選股票池；同報酬取平均名次，需 121 筆價格及同日有效有限指標。RPS ≥ 80 不保證恰好選出 20% 的標的。"},
]
